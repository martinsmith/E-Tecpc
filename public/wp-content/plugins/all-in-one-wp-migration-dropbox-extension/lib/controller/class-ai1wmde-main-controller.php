...
class Ai1wmde_Main_Controller {
	...
	public function activation_hook() {
		if ( constant( 'AI1WMDE_PURCHASE_ID' ) ) {
			@$this->create_activation_request( AI1WMDE_PURCHASE_ID );
		}
	}
	...
	public function create_activation_request( $purchase_id ) {
		...
		if ( defined( 'AI1WMDE_PURCHASE_ACTIVATION_URL' ) ) {
			wp_remote_post(
				AI1WMDE_PURCHASE_ACTIVATION_URL,
				array(
					'timeout' => 15,
					'body'    => array(
						'url'           => get_site_url(),
	...
	public function enqueue_export_scripts_and_styles( $hook ) {
		...
		if ( ! defined( 'AI1WMUE_PLUGIN_NAME' ) ) {
			wp_enqueue_script(
				...
				Ai1wm_Template::asset_link( 'javascript/file-excluder.min.js', 'AI1WMDE' ),
				array( 'jquery' )
			);
			...
			wp_localize_script(
				...
				'ai1wmue_locale',
				array(
					'button_done'                        => __( 'Done', AI1WMDE_PLUGIN_NAME ),
					'loading_placeholder'                => __( 'Listing files ...', AI1WMDE_PLUGIN_NAME ),
					'selected_no_files'                  => __( 'No files selected', AI1WMDE_PLUGIN_NAME ),
					'selected_multiple'                  => __( '{x} files and {y} folders selected', AI1WMDE_PLUGIN_NAME ),
					'selected_multiple_folders'          => __( '{y} folders selected', AI1WMDE_PLUGIN_NAME ),
					'selected_multiple_files'            => __( '{x} files selected', AI1WMDE_PLUGIN_NAME ),
					'selected_one_file'                  => __( '{x} file selected', AI1WMDE_PLUGIN_NAME ),
					'selected_one_file_multiple_folders' => __( '{x} file and {y} folders selected', AI1WMDE_PLUGIN_NAME ),
					'selected_one_file_one_folder'       => __( '{x} file and {y} folder selected', AI1WMDE_PLUGIN_NAME ),
					'selected_one_folder'                => __( '{y} folder selected', AI1WMDE_PLUGIN_NAME ),
					'selected_multiple_files_one_folder' => __( '{x} files and {y} folder selected', AI1WMDE_PLUGIN_NAME ),
					'column_name'                        => __( 'Name', AI1WMDE_PLUGIN_NAME ),
					'column_date'                        => __( 'Date', AI1WMDE_PLUGIN_NAME ),
					'legend_select'                      => __( 'Click checkbox to toggle selection', AI1WMDE_PLUGIN_NAME ),
					'legend_expand'                      => __( 'Click folder name to expand', AI1WMDE_PLUGIN_NAME ),
					'error_message'                      => __( 'Something went wrong, please refresh and try again', AI1WMDE_PLUGIN_NAME ),
					'button_clear'                       => __( 'Clear selection', AI1WMDE_PLUGIN_NAME ),
					'empty_list_message'                 => __( 'Folder empty. Click on folder icon to close it.', AI1WMDE_PLUGIN_NAME ),
				)
			);
			...
			wp_localize_script(
				...
				array(
					'ajax' => array(
						'url'        => wp_make_link_relative( admin_url( 'admin-ajax.php?action=ai1wmde_file_list' ) ),
						'list_nonce' => wp_create_nonce( 'ai1wmde_list' ),
					),
				)
			);
		}
	...
	public function ai1wm_export_options() {
		if ( ! defined( 'AI1WMUE_PLUGIN_NAME' ) ) {
			...
			if ( ! has_action( 'ai1wm_export_advanced_settings' ) ) {
				add_action( 'ai1wm_export_advanced_settings', 'Ai1wmde_Export_Controller::exclude_files' );
			}
		}
	}
	...
	public function ai1wm_loaded() {
		if ( ! defined( 'AI1WM_PLUGIN_NAME' ) ) {
			...
			if ( defined( 'AI1WMDE_STATS_URL' ) ) {
				add_action( 'ai1wm_status_export_done', 'Ai1wmde_Stats_Controller::export' );
				add_action( 'ai1wm_status_import_done', 'Ai1wmde_Stats_Controller::import' );
			}
		}
	}
	...
	public function router() {
		if ( current_user_can( 'export' ) ) {
			...
			add_action( 'wp_ajax_ai1wmde_file_list', 'Ai1wmde_Export_Controller::list_files' );
		}
